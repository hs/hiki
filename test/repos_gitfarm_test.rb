...
class Repos_Gitfarm_Tests < Test::Unit::TestCase
  ...
  def setup
    ...
    @repos = Hiki::ReposGitfarm.new(@root, "#{@data_dir}/#{@wiki}")
  ...
  def test_get_revision
    ...
    Dir.chdir(@data_dir) {rev1 = git("hash-object", "#{@wiki}/text/HogeHoge")}
    write("HogeHoge", 'hogehoge2')
    Dir.chdir(@data_dir) {git("commit", "-m", "Second", "#{@wiki}/text/HogeHoge")}
    Dir.chdir(@data_dir) {rev2 = git("hash-object", "#{@wiki}/text/HogeHoge")}
    write("HogeHoge", 'hogehoge3')
    Dir.chdir(@data_dir) {git("commit", "-m", "Third", "#{@wiki}/text/HogeHoge")}
    Dir.chdir(@data_dir) {rev3 = git("hash-object", "#{@wiki}/text/HogeHoge")}
